            ...
            <ul class="layui-nav layui-nav-tree" lay-filter="test">
                ...
                <li class="layui-nav-item layui-nav-itemed">
                    ...
                    <dl class="layui-nav-child">
                        ...
                        <dd><a href="/backstageindex/messagemanagement"  style="color: #978502">留言管理</a></dd>
                    </dl>
                </li>
            </ul>
        </div>
...
<script>
    layui.use('table', function () {
        ...
        table.render({
            elem: '#test'
            , url: '/backstageindex/messagelist'
            , toolbar: '#toolbarDemo'
            , height: '558px'
            , cols: [[
                {type: 'checkbox', width: 60, unresize: true}
                , {field: 'uuidMessage', width: 90, title: '留言ID', sort: true, unresize: true}
                , {field: 'messageUserName', width: 160, title: '留言用户', unresize: true}
                , {field: 'messageContent', Width: 120, title: '留言内容', unresize: true, sort: true}
                , {field: 'messageDate', width: 140, title: '留言时间', unresize: true, sort: true}
        ...
        table.on('tool(test)', function (obj) {
            var data = obj.data;
            if (obj.event === 'del') {
                layer.confirm('真的删除' + obj.data.messageUserName + '的留言吗？'
                    , function (index) {
                        layer.close(index);
                        critiqueDeleteAll(obj.data.uuidMessage);
                        obj.del();
                    });
                ...
                var setb = layer.open({
                    ...
                    content: "<div style='width: 1080px;height: 290px;background-color: white;position: relative;'>" +
                        "<div class='layui-form-item' style='margin: 15px 0 15px 340px;display: inline-block'>" +
                        "<label class='layui-form-label'>留言用户</label>" +
                        "<div class='layui-input-block'>" +
                        "<input style='width: 320px' value='" + obj.data.messageUserName + "' type='text' name='title' required disabled  lay-verify='required' placeholder='请输入标题' autocomplete='off' class='layui-input layui-btn-disabled'>" +
                        "</div>" +
                        "</div>" +
                ...
                $('#modifywz').click(function () {
                    ...
                    var copyNr = obj.data.messageContent;
                    var cuuid = obj.data.uuidMessage;

                    console.log(obj.data.uuidMessage)
                    ...
                    if (copyNr === nr) {
                        ...
                        if (nr !== null && nr.length <=30) {
                            $.ajax({
                                type: "POST",
                                url: "/backstageindex/messagemodification",
                            ...
                            layer.alert("留言内容不能为空，且长度不能超过30位");
                        }
                    }
                });
            }
                ...
                case 'addCheckData':
                    ...
                    if (data.length === 0) {
                        ...
                        $.each(data, function (index, obj) {
                            critiqueDeleteAll(obj.uuidMessage);
                        });
                        table.reload('test');
                        break;
                    }
    ...
    function critiqueDeleteAll(uuid) {
        $.ajax({
            type: "POST",
            url: "/backstageindex/messageDeleteAll",
            contentType: "application/x-www-form-urlencoded; charset=utf-8",
            data: {
                "uuid": uuid,
            },
